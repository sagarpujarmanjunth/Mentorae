...
function displayArtifactResult(searchData, userMessage) {
    ...
    if (searchData.results && searchData.results.length > 0) {
        ...
        searchData.results.slice(0, 6).forEach((result, index) => {
            ...
            if (result.domain.includes('wikipedia')) resultIcon = 'fab fa-wikipedia-w';
            ...
            else if (result.domain.includes('geeksforgeeks')) resultIcon = 'fas fa-code';
...
`;
    ...
    try {
        ...
        console.error("Error displaying Claude-style artifact, falling back to text-only display:", error);
...
function sendMessage() {
    ...
    const shouldShowSearchArtifact = !document.querySelector('.status-dot.active'); // Show if no RAG loaded
        ...
        .then(searchResponse => {
            if (searchResponse.success && searchResponse.search_data) {
                ...
                if (searchResponse.engine_used) {
                    showToast(`Search powered by ${searchResponse.engine_used}`, 'info');
                }
                ...
                if (searchResponse.timeout) {
                    showToast('Search timed out. Switching to backup method...', 'warning');
...
function createModernScrapedInfo(scrapedData) {
    ...
    lines.forEach(line => {
        ...
        // Check for direct URLs
            ...
            if (urlMatch) {
                const [, number, url] = urlMatch;
                const existingSource = sources.find(s => s.number === parseInt(number));
                if (existingSource) {
        ...
        <div class="modern-scraped-container">
            <div class="modern-scraped-header" onclick="toggleModernScrapedInfo(this)">
                <div class="scraped-header-content">
                    ...
                    ${sources.length > 0 ? `<div class="scraped-count">${sources.length} sources</div>` : ''}
                </div>
...
function createSourceItem(source) {
    ...
    return `
        <div class="source-item" onclick="openCitationLink('${source.url}', '${escapeHtml(source.title)}')">
            ...
            <div class="source-content">
                <div class="source-title">${truncateText(source.title, 60)}</div>
...
function getDomainSpecificIcon(domain) {
    ...
    if (domain.includes('geeksforgeeks')) return 'fas fa-code';
    if (domain.includes('w3schools')) return 'fas fa-code';
    if (domain.includes('mdn') || domain.includes('mozilla')) return 'fab fa-firefox';
    return 'fas fa-globe';
}
...
function constructUrlFromSource(title, domain) {
    ...
    return domain ? `https://${domain}` : `https://www.google.com/search?q=${encodeURIComponent(title)}`;
}
